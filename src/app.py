...
def get_people(id):
    try:
        ...
        print("valor de search:", search_serialize)
...
def add_people():
    try:
        ...
        return jsonify({"message":"El personaje se agrego correctamente"}), 200
    except Exception as error:
        print(error)
        return jsonify({"message":str(error)}), 500
...
def edit_people(id):
    try:
        ...
        return jsonify({"message":"El personaje se edito correctamente"}), 200
    except Exception as error:
        print(error)
        return jsonify({"message":str(error)}), 500
...
def delete_people(id):
    try:
        ...
        return jsonify({"message":"El personaje se elimino correctamente"}), 200
    except Exception as error:
        print(error)
        return jsonify({"message":str(error)}), 500
